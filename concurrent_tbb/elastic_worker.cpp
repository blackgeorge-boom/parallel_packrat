...
#include "elastic_worker.h"

ElasticWorker::ElasticWorker(std::string input, const PEG& g, Cell** c, int p)
{
    in = std::move(input);
    peg = PEG(g);
    cells = c;
...
bool ElasticWorker::visit(NonTerminal &nt)
{
    int row = nt.index();
...
bool ElasticWorker::visit(CompositeExpression &ce)
{
...
bool ElasticWorker::visit(Terminal& t)
{
//    if (stopRequested()) {//TODO:
//        std::cout << "Stopped\n";
//        return false;
